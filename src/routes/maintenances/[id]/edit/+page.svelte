<script lang="ts">
	...
	async function loadMaintenance() {
		try {
			...
			formData = {
				...
				category: (maintenance.category || 'other') as MaintenanceCategory,
				description: maintenance.description || '',
				cost: formatCurrency(maintenance.cost.toString()),
				km_at_service: formatKm(maintenance.km_when_done?.toString() || '0'),
				service_date: maintenance.service_date.split('T')[0],
				next_service_km: maintenance.next_km ? formatKm(maintenance.next_km.toString()) : '',
				...
				warranty_until: maintenance.warranty_until ? maintenance.warranty_until.split('T')[0] : ''
			};

			// Show advanced if any advanced field has data
			if (
				formData.next_service_km ||
	...
	async function handleSubmit(e: Event) {
		...
		try {
			...
			await maintenancesApi.update(maintenance.id, submitData, token);
			goto(`/maintenances/${maintenance.id}`);
		} catch (err: any) {
	...
	<DashboardLayout>
		<div class="mx-auto max-w-4xl space-y-6">
			...
			<div class="flex items-center gap-4">
				<a
					href="/maintenances/{$page.params.id}"
				...
				<div>
					<h1 class="text-2xl font-bold text-gray-900 dark:text-white">Editar Manutenção</h1>
