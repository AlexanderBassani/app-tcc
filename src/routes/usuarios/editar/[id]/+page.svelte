<script lang="ts">
	...
	async function loadUserData() {
		...
		try {
			...
			const response = await usersApi.getById(userId, $authStore.token);
			...
			formData.phone = user.phone ? formatPhone(user.phone) : '';
			// Converter data de ISO para formato YYYY-MM-DD
			formData.date_of_birth = user.date_of_birth ? user.date_of_birth.split('T')[0] : '';
			formData.gender = (user.gender as any) || '';
			formData.bio = user.bio || '';
			formData.status = user.status || 'active';
			formData.preferred_language = user.preferred_language || 'pt-BR';
			formData.timezone = user.timezone || 'America/Sao_Paulo';
			formData.marketing_emails_consent = user.marketing_emails_consent || false;
		} catch (e: any) {
			console.error('Erro ao carregar usuário:', e);
			errorMessage = e.response?.data?.error || 'Erro ao carregar dados do usuário';
		} finally {
			isLoading = false;
		}
	}
	...
	async function handleSubmit(e: Event) {
		...
		// Validação de senha apenas se for informada
		...
		try {
			if (!$authStore.token) {
				errorMessage = 'Você precisa estar autenticado para atualizar usuários';
				return;
			}
			...
			// Chamar API
			const response = await usersApi.update(userId, payload, $authStore.token);
...
<DashboardLayout>
	<div class="space-y-6">
		<!-- Header -->
		<div>
			<h2 class="text-2xl font-bold text-gray-800 dark:text-white">Editar Usuário</h2>
		...
		{#if isLoading}
			<div
				class="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-blue-800 dark:border-blue-800 dark:bg-blue-900/20 dark:text-blue-300"
			>
