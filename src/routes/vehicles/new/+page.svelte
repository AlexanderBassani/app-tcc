<script lang="ts">
	...
	let plateValid = $state(false);
	let plateMessage = $state('');
	let plateAvailable = $state<boolean | null>(null);
	...
	async function handleSubmit(e: Event) {
		...
		try {
			...
			await vehiclesApi.create(submitData, token);
			goto('/vehicles');
		} catch (err: any) {
...
<DashboardLayout>
	<div class="mx-auto max-w-4xl space-y-6 px-4 py-6">
		...
		<div class="flex items-center gap-4">
			...
			<div>
				<h1 class="text-3xl font-bold text-gray-800 dark:text-white">Cadastrar Veículo</h1>
				<p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
					Adicione um veículo à sua garagem
				</p>
			</div>
		</div>
